...
def load_params_from_history(row):
    ...
    # Mapping table columns to sidebar widget keys
    mapping = {
        'model': 'model_select',
    ...
    for col, state_key in mapping.items():
        if col in row and row[col] is not None:
            ...
            if col == 'n_hill':
                ...
                try:
                    val = row[col]
                    # Ensure types match what widgets expect (int vs float)
...
def on_rerun_click():
    """
    Callback function attached to the 'Rerun' button.
    This runs BEFORE the rest of the script reruns, avoiding the 'set widget after render' error.
    """
    # 1. Get the selected row index from the dataframe state
    if "history_table" in st.session_state and st.session_state["history_table"]["selection"]["rows"]:
        ...
        # 2. Get the actual data row
        if idx < len(st.session_state.run_history):
            selected_row = st.session_state.run_history[idx]
            
            # 3. Update the session state (Sidebar widgets will read these new values)
            load_params_from_history(selected_row)
            
            # 4. Set the trigger flag so main() knows to run the sim
