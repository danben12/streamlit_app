...
# ==========================================
# 4. UI COMPONENTS (SIDEBAR WITH FORM)
# ==========================================
...
def render_sidebar():
    ...
    with col_t1:
        params['t_start'] = st.number_input("Start (h)", value=0.0, step=1.0, key='t_start')
    with col_t2:
        params['t_end'] = st.number_input("End (h)", value=24.0, step=1.0, key='t_end')
    with col_t3:
        params['dt'] = st.number_input("Step (h)", value=1.0, min_value=0.01, step=0.5, key='dt')
...
# ==========================================
# 5. CORE LOGIC & CALCULATION FUNCTIONS
# ==========================================
...
def plot_fold_change(vols, initial_biomass, final_biomass, vc_val):
    ...
    p.add_tools(
        HoverTool(tooltips=[('Volume', '@Volume{0,0}'), ('Fold Change', '@FoldChange{0.00}'), ('ID', '@DropletID')],
                  renderers=[r_scat]))
...
def main():
    ...
    if "last_selection_ts" not in st.session_state: st.session_state.last_selection_ts = None

    # --- CRITICAL FIX: CHECK FOR HISTORY SELECTION *BEFORE* SIDEBAR RENDERS ---
    # We access the dataframe state directly from session_state before the widget is drawn again
    if "history_table" in st.session_state and st.session_state["history_table"]["selection"]["rows"]:
        selected_indices = st.session_state["history_table"]["selection"]["rows"]
        if selected_indices:
            row_idx = selected_indices[0]
            # Safety check: ensure index is valid
            if row_idx < len(st.session_state.run_history):
                selected_row = st.session_state.run_history[row_idx]
                ts = selected_row.get('Timestamp')
                
                # Only load if this is a NEW selection to avoid infinite reload loops
                if ts != st.session_state.last_selection_ts:
                    st.session_state.last_selection_ts = ts
                    load_params_from_history(selected_row)
                    # We do NOT return here, we let the script flow continue so sidebar uses new values
        ...
        with st.spinner("Running simulation..."):
            ...
            if N_occupied > 0:
                ...
                if sim_output:
                    ...
                    # Add to history if unique (check timestamp of last entry)
    ...
    with tab_hist:
        ...
        st.info("👆 **Click on a row** in the table below to restore its parameters and **automatically rerun** the simulation.")
        
        if st.session_state.run_history:
            df_hist = pd.DataFrame(st.session_state.run_history)
            
            # Formatting: Reorder columns
            ...
            # Note: The logic to handle this selection is now at the TOP of main()
